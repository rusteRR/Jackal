...
namespace jackal {
    ...
    SimplePointerType Settings::get_simple_pointer() {
        std::random_device rd;
        std::default_random_engine eng(rd());
        std::uniform_int_distribution<int> distr(0, static_cast<int> (m_simple_pointer.size()));
        int index = distr(eng);
    ...
    MultiPointerType Settings::get_multi_pointer() {
        std::random_device rd;
        std::default_random_engine eng(rd());
        std::uniform_int_distribution<int> distr(0, static_cast<int> (m_multi_pointer.size()));
        int index = distr(eng);
